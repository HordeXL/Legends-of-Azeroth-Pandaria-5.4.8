...
- Restart verification completed on 2026-07-16 at 10:20. `Server.log` reports `World initialized in 0 minutes 24 seconds`. `DBErrors.log` was appended rather than truncated: lines 1-7,631 belong to the 09:35 start, while the new start is lines 7,632-15,255 (7,624 lines). Counts must therefore be taken only from the new segment.
- The new segment contains zero `ObjectEntryGuid condition has non existing creature template` warnings. All eight old references to `31317`, `33906`, `33909`, and `40684` are confined to the earlier segment, so the four-template restoration is verified. The already repaired `27754`, Shoeshine/`9603600`, and questgiver-flag groups also remain at zero in the new segment.
- Restoration exposed one narrower follow-up validator warning: parent `32930` and difficulty entry `33909` had different `npcflag` values. Both pre-fix world dumps preserve `npcflag=16777216` on the active Build-15595 parent, while the restored child had `0`. Exact pre-change rows were saved to `world_creature_template_32930_33909_before_npcflag_20260716.sql`.
- Added and applied guarded update `2026_07_16_01_world_align_kologarn_difficulty_npcflag.sql`. It changes only child `33909` from `0` to the linked parent value when both expected Build-15595 rows and the exact `32930 -> 33909` relationship exist. Transaction validation with `ROLLBACK` passed first; the post-apply query confirms both rows now have `npcflag=16777216`. No row was deleted. One more restart is required to confirm that the single new difficulty warning is gone.
