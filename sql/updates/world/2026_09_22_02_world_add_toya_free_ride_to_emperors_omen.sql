...
-- The ride option is gated on quest 29925 being taken (any state): Toya is
-- both the quest giver and the ride NPC, so the "I'm ready to see Lorewalker
-- Cho." option must only appear once the quest is in the log.  QUESTTAKEN
-- would be wrong here: 29925 is a delivery quest whose ITEM objective is
-- satisfied the moment the quest is accepted (StartItem 76483 is added to
-- the inventory), so the status is immediately QUEST_STATUS_COMPLETE, and
-- CONDITION_QUESTTAKEN only matches QUEST_STATUS_INCOMPLETE.  Use
-- CONDITION_QUEST_NONE negated instead ("quest is not untaken").
DELETE FROM `conditions` WHERE `SourceTypeOrReferenceId` = 15 AND `SourceGroup` = 56348 AND `SourceEntry` = 0 AND `SourceId` = 0 AND `ElseGroup` = 0;
INSERT INTO `conditions` (`SourceTypeOrReferenceId`, `SourceGroup`, `SourceEntry`, `SourceId`, `ElseGroup`, `ConditionTypeOrReference`, `ConditionTarget`, `ConditionValue1`, `ConditionValue2`, `ConditionValue3`, `NegativeCondition`, `ErrorType`, `ErrorTextId`) VALUES
(15, 56348, 0, 0, 0, 14, 0, 29925, 0, 0, 1, 0, 0);
