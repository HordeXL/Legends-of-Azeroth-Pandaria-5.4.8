...
-- 2026_07_29_02_world_split_first_two_hundred_cross_map_master_pools.sql.

DROP TEMPORARY TABLE IF EXISTS `_tmp_first_200_master_rollback_maps_20260729`;
CREATE TEMPORARY TABLE `_tmp_first_200_master_rollback_maps_20260729`
(
...
INSERT IGNORE INTO `_tmp_first_200_master_rollback_maps_20260729`
...
INSERT INTO `_tmp_first_200_master_rollback_maps_20260729`
(`old_mother`,`child_pool`,`map_id`)
...
DROP TEMPORARY TABLE IF EXISTS `_tmp_first_200_master_rollback_splits_20260729`;
CREATE TEMPORARY TABLE `_tmp_first_200_master_rollback_splits_20260729`
(
    `old_mother` MEDIUMINT UNSIGNED NOT NULL,
    `map_id` SMALLINT UNSIGNED NOT NULL,
...
INSERT INTO `_tmp_first_200_master_rollback_splits_20260729`
...
FROM `_tmp_first_200_master_rollback_maps_20260729` AS `lm`
...
SET @first_200_master_rollback_backup_ok :=
(
    SELECT
        (SELECT COUNT(*)
         FROM `_backup_pool_manifest_first_200_master_20260729`) = 200
        AND
        (SELECT COUNT(*)
         FROM `_backup_pool_template_first_200_master_20260729`) = 200
        AND
        (SELECT COUNT(*)
         FROM `_backup_pool_pool_first_200_master_20260729`) = 800
        AND
        (SELECT COUNT(*)
         FROM `_tmp_first_200_master_rollback_maps_20260729`) = 800
        AND
        (SELECT COUNT(*)
         FROM `_tmp_first_200_master_rollback_splits_20260729`) = 227
);

SET @first_200_master_rollback_links_ok :=
(
    SELECT COUNT(*) = 800
       AND SUM(
    ...
    INNER JOIN `_tmp_first_200_master_rollback_maps_20260729` AS `lm`
        ON `lm`.`old_mother` = `b`.`mother_pool`
       AND `lm`.`child_pool` = `b`.`pool_id`
    LEFT JOIN `_tmp_first_200_master_rollback_splits_20260729` AS `s`
        ON `s`.`old_mother` = `b`.`mother_pool`
       AND `s`.`map_id` = `lm`.`map_id`
);

SET @first_200_master_rollback_no_foreign_ok :=
(
    SELECT COUNT(*) = 0
    FROM `pool_pool` AS `pp`
    INNER JOIN `_tmp_first_200_master_rollback_splits_20260729` AS `s`
...
SET @first_200_master_rollback_templates_ok :=
(
    SELECT COUNT(*) = 0
...
SET @first_200_master_rollback_ok :=
    @first_200_master_rollback_backup_ok
    AND @first_200_master_rollback_links_ok
    AND @first_200_master_rollback_no_foreign_ok
    AND @first_200_master_rollback_templates_ok;
...
WHERE @first_200_master_rollback_ok = 1;
...
SELECT @first_200_master_rollback_ok AS `rollback_guard`,
       (SELECT COUNT(*)
       ...
       (SELECT COUNT(*)
        FROM `_backup_pool_pool_first_200_master_20260729`)
           AS `link_backup_rows`,
       (SELECT COUNT(*)
        FROM `_tmp_first_200_master_rollback_splits_20260729`)
           AS `new_mother_count`;

DROP TEMPORARY TABLE `_tmp_first_200_master_rollback_splits_20260729`;
DROP TEMPORARY TABLE `_tmp_first_200_master_rollback_maps_20260729`;
