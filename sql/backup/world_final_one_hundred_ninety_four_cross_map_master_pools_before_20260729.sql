...
-- 2026_07_29_04_world_split_final_one_hundred_ninety_four_cross_map_master_pools.sql.

DROP TEMPORARY TABLE IF EXISTS `_tmp_final_194_master_rollback_maps_20260729`;
CREATE TEMPORARY TABLE `_tmp_final_194_master_rollback_maps_20260729`
(
...
INSERT INTO `_tmp_final_194_master_rollback_maps_20260729`
...
DROP TEMPORARY TABLE IF EXISTS `_tmp_final_194_master_rollback_splits_20260729`;
CREATE TEMPORARY TABLE `_tmp_final_194_master_rollback_splits_20260729`
(
    `old_mother` MEDIUMINT UNSIGNED NOT NULL,
    `map_id` SMALLINT UNSIGNED NOT NULL,
...
INSERT INTO `_tmp_final_194_master_rollback_splits_20260729`
...
FROM `_tmp_final_194_master_rollback_maps_20260729` AS `lm`
...
SET @final_194_master_rollback_backup_ok :=
(
    SELECT
        (SELECT COUNT(*)
         FROM `_backup_pool_manifest_final_194_master_20260729`) = 194
        AND
        (SELECT COUNT(*)
        ...
        AND
        (SELECT COUNT(*)
         FROM `_tmp_final_194_master_rollback_maps_20260729`) = 774
        AND
        (SELECT COUNT(*)
         FROM `_tmp_final_194_master_rollback_splits_20260729`) = 195
);

SET @final_194_master_rollback_links_ok :=
(
    SELECT COUNT(*) = 774
       AND SUM(
    ...
    INNER JOIN `_tmp_final_194_master_rollback_maps_20260729` AS `lm`
        ON `lm`.`old_mother` = `b`.`mother_pool`
       AND `lm`.`child_pool` = `b`.`pool_id`
    LEFT JOIN `_tmp_final_194_master_rollback_splits_20260729` AS `s`
        ON `s`.`old_mother` = `b`.`mother_pool`
       AND `s`.`map_id` = `lm`.`map_id`
);

SET @final_194_master_rollback_no_foreign_ok :=
(
    SELECT COUNT(*) = 0
    FROM `pool_pool` AS `pp`
    INNER JOIN `_tmp_final_194_master_rollback_splits_20260729` AS `s`
...
SET @final_194_master_rollback_templates_ok :=
(
    SELECT COUNT(*) = 0
...
SET @final_194_master_rollback_ok :=
    @final_194_master_rollback_backup_ok
    AND @final_194_master_rollback_links_ok
    AND @final_194_master_rollback_no_foreign_ok
    AND @final_194_master_rollback_templates_ok;
...
WHERE @final_194_master_rollback_ok = 1;
...
SELECT @final_194_master_rollback_ok AS `rollback_guard`,
       (SELECT COUNT(*)
       ...
       (SELECT COUNT(*)
        FROM `_backup_pool_pool_final_194_master_20260729`)
           AS `link_backup_rows`,
       (SELECT COUNT(*)
        FROM `_tmp_final_194_master_rollback_splits_20260729`)
           AS `new_mother_count`;

DROP TEMPORARY TABLE `_tmp_final_194_master_rollback_splits_20260729`;
DROP TEMPORARY TABLE `_tmp_final_194_master_rollback_maps_20260729`;
